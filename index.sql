...
SELECT COUNT(DISTINCT FirstName) FROM company.employees;

-- 21. Select all employees and their corresponding sales 
SELECT e.FirstName, e.LastName, p.ProductName, s.Total
FROM company.sales s
JOIN company.employees e ON s.EmployeeID = e.EmployeeID
JOIN company.products p ON s.ProductID = p.ProductID;

-- 22. Select the first 10 employees hired, ordered by their HireDate.
SELECT * FROM company.employees ORDER BY HireDate
LIMIT 10;

-- 23. Select the employees who have not made any sales. 

SELECT * FROM company.sales WHERE Total IS NULL;
...
SELECT Department, AVG(Quantity) AS AvgQuantity
FROM (
    SELECT s.EmployeeID, e.Department, s.Quantity
    FROM company.sales s, company.employees e
...
SELECT Category, SUM(Total) AS TotalRevenue
FROM (
    SELECT s.ProductID, p.Category, s.Total
    FROM company.sales s, company.products p
